...
export class MultiselectRecordsEntity
  implements ComponentFramework.StandardControl<IInputs, IOutputs>
{
  ...
  private _filterDynamicFieldValue: string;
  ...
  public init(
    ...
    this._filterDynamicFieldValue = this._context.parameters.filterDynamicField.raw || "";
  ...
  private async filteredUrlFromDynamicValues(_filter: string): Promise<string> {
    ...
    if (this._entityRecordId != undefined && this._entityRecordId != "") {
      ...
      arrayDynamicValues.forEach((value: string, index: number) => {
        index++;
        var apiValue = this._filterDynamicFieldValue;
        var replaceindex = `${index}`;
        var regex = new RegExp("\\{" + replaceindex + "\\}", "g");
        _filter = _filter.replace(regex, apiValue);
        return;
  ...
  private renderElement(): void {
    ReactDOM.render(React.createElement(MultiselectRecords, this.props), this._container);
  }
